...
def trackclouds(zipped_inputs):
    ...
    if hour_diff < timegap and hour_diff > 0:
        ...
        if nreference.size != 0:
            ...
            logger.info("nreference: ", nreference)
        else:
            nreference = 0
        ...
        if nnew.size != 0:
            nnew = nnew[-1]
            logger.info("nnew: ", nnew)
        else:
            nnew = 0

        # logger.info(type(nreference))
        ############################################################
        # Get size of data
        ...
        output_data.newcloud_backward_index.attrs["valid_max"] = np.max(nreference)
        ...
        output_data.refcloud_forward_index.attrs["valid_max"] = np.max(nnew)
