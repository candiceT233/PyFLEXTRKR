...
def map_ct(zipped_inputs):
    ...
    if showalltracks == 0:
        statistics_file = (
            stats_path + trackstats_filebase + "_" + startdate + "_" + enddate + ".nc"
        )
    ...
    celltrackmaps_outfile = (
        cttracking_path + "cttracks_" + str(filedate) + "_" + str(filetime) + ".nc"
    )
    ...
    if showalltracks == 0:
        output_data = xr.Dataset(
            {
                ...
                "ctsplittracknumbers": (
                    ["time", "nlat", "nlon"],
                    np.expand_dims(allsplitmap, axis=0),
                ),
                "ctmergetracknumbers": (
                    ["time", "nlat", "nlon"],
                    np.expand_dims(allmergemap, axis=0),
                ),
                "cloudnumber": (["time", "nlat", "nlon"], convcold_cloudnumber),
                "cttracknumber": (
                    ["time", "nlat", "nlon"],
                    np.expand_dims(trackmap, axis=0),
                ),
            },
            coords={
            ...
            attrs={
                "title": "Pixel level of tracked clouds",
                "source": datasource,
        ...
        output_data.cttracknumber.attrs[
            "long_name"
