...
def filtermcs_wrf_rain(
    ...
    for nt in range(0, ntracks):
        ...
        ipfmcs = np.array(
            np.where((ipf_majoraxis > majoraxisthresh) & (ipf_dbz50area > 0))
        )[0, :]
        nipfmcs = len(ipfmcs)
            ...
            if nipfmcs * time_res > durationthresh:
                ...
                for igroup in range(0, nbreaks):
                    ...
                    if np.multiply(len(groups[igroup][:]), time_res) > durationthresh:
                        ...
                        igroup_ccaspectratio = np.copy(
                            ipf_ccaspectratio[igroup_indices]
                        )
    ...
    if nlongmcs > 0:
        ...
        for ilm in range(0, nlongmcs):
            ...
            ilm_pfccmajoraxis = np.copy(
                pf_ccmajoraxis[ilongmcs[ilm], 0:ilm_irtracklength, :]
            )
            ...
            if nccline > 1:
                ...
                if iccline[1] > iccarea[0]:
                    ...
                    ilm_cycle[
                        iccarea[0] : iccline[1]
                    ] = 2  # Time period of unorganzied convective cells
                else:
                    sys.exit(
                ...
                if nsfarea > 0:
                    ...
                    if isfarea[0] > iccline[1] + 2:
                        ...
                        ilm_cycle[
                            iccline[1] : isfarea[0]
                        ] = 3  # Time period of organized cells before maturation

                        ilm_cycle[
                            isfarea[0] : isfarea[-1] + 1
                        ] = 4  # Time period of mature mcs
                    else:
                        matureindex = isfarea[
                            np.array(np.where(isfarea == iccline[1] + 2))[0, :]
                        ]
                        if len(matureindex) > 0:
                            ilm_index[3] = np.copy(matureindex[0])
                            ilm_cycle[
                                iccline[1] : matureindex[0]
                            ] = 3  # Time period of organized cells before maturation

                            ilm_cycle[
                                matureindex[0] : isfarea[-1] + 1
                            ] = 4  # Time period of mature mcs
                            ...
                            ilm_cycle[
                                isfarea[-1] + 1 : ilm_irtracklength + 1
                            ] = 5  # Time period of dissipation
    ...
    output_data = xr.Dataset(
        {
            "mcs_length": (["track"], data["length"][trackid_mcs]),
        ...
        coords={
            "track": (["track"], np.arange(1, len(trackid_mcs) + 1)),
        ...
        attrs={
            "title": "Statistics of MCS definedusing NMQ precipitation features",
    ...
    output_data.pf_mcsstatus.attrs[
        "description"
    ] = "Flag indicating if this time part of the MCS 1 = Yes, 0 = No"
    output_data.pf_mcsstatus.attrs["units"] = "unitless"

    output_data.lifecycle_complete_flag.attrs[
        "description"
    ] = "Flag indicating if this MCS has each element in the MCS life cycle"
    output_data.lifecycle_complete_flag.attrs["units"] = "unitless"

    output_data.lifecycle_index.attrs[
        "description"
    ] = "Time index when each phase of the MCS life cycle starts"
    output_data.lifecycle_index.attrs["units"] = "unitless"

    output_data.lifecycle_stage.attrs[
        "description"
    ] = "Each time in the MCS is labeled with a flag indicating its phase in the MCS lifecycle. 1 = Cloud only, 2 = Isolated convective cores, 3 = MCS genesis, 4 = MCS maturation, 5 = MCS decay"
