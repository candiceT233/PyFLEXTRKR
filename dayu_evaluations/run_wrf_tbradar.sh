...
config_example='config_wrf_mcs_tbradar_template.yml'
# config_example='config_wrf_mcs_tbradar_seq.yml'
config_demo='config_wrf_mcs_tbradar.yml'
...
# check number of tasks and nodes
...
RUN_TRACKING () {
    ...
    # srun -N$SLURM_JOB_NUM_NODES -n$SLURM_NTASKS -w $hostlist python ../runscripts/run_mcs_tbpfradar3d_wrf.py ${config_demo} &> ${FUNCNAME[0]}-${TEST_NAME}.log
    # srun -n$SLURM_NTASKS -w $hostlist python ../runscripts/run_mcs_tbpfradar3d_wrf.py ${config_demo} &> ${FUNCNAME[0]}-${TEST_NAME}.log
    # srun -n$SLURM_NTASKS --ntasks-per-node=${SLURM_NTASKS_PER_NODE} -w $hostlist python ../runscripts/run_mcs_tbpfradar3d_wrf.py ${config_demo} &> ${FUNCNAME[0]}-${TEST_NAME}.log

    # srun -n$SLURM_NTASKS --ntasks-per-node=${SLURM_NTASKS_PER_NODE} -w $hostlist python ../runscripts/run_mcs_tbpfradar3d_wrf.py ${config_demo} &> ${FUNCNAME[0]}-${TEST_NAME}.log

    srun -N$SLURM_JOB_NUM_NODES -n$SLURM_NTASKS -w $hostlist --oversubscribe python ../runscripts/run_mcs_tbpfradar3d_wrf.py ${config_demo} &> ${FUNCNAME[0]}-${TEST_NAME}.log
